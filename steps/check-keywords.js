...
const keywords=[...STRONG_PROCUREMENT_TRIGGERS.english, ...STRONG_PROCUREMENT_TRIGGERS.spanish, ...STRONG_PROCUREMENT_TRIGGERS.portuguese, ...PCI_COMPLIANCE_SIGNALS.primary, ...PCI_COMPLIANCE_SIGNALS.secondary, ...NOISE_EXCLUSION_LIST.english, ...NOISE_EXCLUSION_LIST.spanish, ...NOISE_EXCLUSION_LIST.portuguese];
const links = [
    "https://comprar.gob.ar/ComprasElectronicas.aspx?qs=uZmpNBcarZh/t6e8LDmVN|XFAS9B6k9ddqFj4heYwZ4=",
    "javascript:__doPostBack('ctl00$CPH1$CtrlBusquedasHome$btnBusquedaProcesos','')",
    "javascript:__doPostBack('ctl00$CPH1$CtrlConsultasFrecuentes$btnProcesoCompraProximo','')",
    "javascript:__doPostBack('ctl00$CPH1$CtrlConsultasFrecuentes$btnProcesoCompraTreintaDias','')",
    "javascript:__doPostBack('ctl00$CPH1$CtrlConsultasFrecuentes$gvListadoPliegos$ctl02$lnkNumeroProceso','')",
    "javascript:__doPostBack('ctl00$CPH1$CtrlConsultasFrecuentes$gvListadoPliegos$ctl03$lnkNumeroProceso','')",
    "javascript:__doPostBack('ctl00$CPH1$CtrlConsultasFrecuentes$gvListadoPliegos$ctl04$lnkNumeroProceso','')",
    "javascript:__doPostBack('ctl00$CPH1$CtrlConsultasFrecuentes$gvListadoPliegos$ctl05$lnkNumeroProceso','')",
    "javascript:__doPostBack('ctl00$CPH1$CtrlConsultasFrecuentes$gvListadoPliegos$ctl06$lnkNumeroProceso','')",
    "https://comprar.gob.ar/Compras.aspx?qs=W1HXHGHtH10="
];

function checkLinksForKeywords(links, keywords) {
    if (!Array.isArray(links) || !Array.isArray(keywords) || keywords.length === 0) return [];
    ...
    return links.filter(link => {
        ...
        const matchedTerms = keywords.filter(keyword => 
            new RegExp(keyword.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i').test(link)
        );
        return {
            link: link,
