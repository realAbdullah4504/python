                ...
                if 'Total:' in second_cell:
                    ...
                    if year_match:
                        ...
                        if year in target_years:
                            amount_due_cell = cells[-1].get_text(strip=True)
...
async def collect_property_ids(page) -> list:
    ...
    while True:
        ...
        try:
            ...
            print(f"    Grid found, waiting for rows to render...")
        except PlaywrightTimeout:
            print(f"    Grid not found on page {page_num}")
            break

        # Retry up to 20 seconds for onclick rows to appear
        for attempt in range(10):
            await asyncio.sleep(2)
            row_count = await page.evaluate("() => document.querySelectorAll('tr[onclick]').length")

            print(f"    Attempt {attempt+1}: {row_count} rows found")
            if row_count > 0:
                break
        else:
            print("    No rows appeared after 20 seconds")
            grid_html = await page.evaluate("() => document.getElementById('grid')?.innerHTML?.substring(0,300) || 'not found'")
            ...
            if properties:
                ...
                for prop in properties:
                    ...
                    if prop_type == "R":
                        ...
                        print(f"      Skipping type={prop_type}")
            else:
